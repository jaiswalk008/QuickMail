...
const MailHeader = (props:{type:string}) =>{
    ...
    const {recievedEmailsClone} = useSelector((state:any) => state.email);
    //creating emailList using useMemo so that recievedEmails doesnot changes
    // const emailList = useMemo(() => recievedEmails,[]);
    
    useSearch(recievedEmailsClone , searchInput);
    
    const logoutHandler = useCallback(()=>{
        dispatch(authActions.logout());
        dispatch(emailActions.setRecievedEmails([]));
    ...
    return (
        ...
        < div className="d-flex justify-content-end">
           {props.type==="recieved" &&  <div className="search-container">
                <input
                    id="search"
                    type="text"
                    autoComplete="off"
